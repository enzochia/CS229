...
def choose_action(state, mdp_data):
    ...
    for idx_action in range(2):
        for idx_next_state in range(mdp_data['value'].shape[0]):
            action_values[idx_action] += mdp_data['transition_probs'][idx_next_state, state, idx_action] * \
                                         mdp_data['value'][idx_next_state]
...
def update_mdp_transition_counts_reward_counts(mdp_data, state, action, new_state, reward):
    ...
    # *** START CODE HERE ***
    mdp_data['transition_counts'][new_state, state, action] += 1
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    # *** START CODE HERE ***
    for state in range(mdp_data['reward_counts'].shape[0]):
        if mdp_data['reward_counts'][state, 1] > 0:
            mdp_data['reward'][state] = (-mdp_data['reward_counts'][state, 0]) / \
                                        mdp_data['reward_counts'][state, 1]

    total_count = mdp_data['transition_counts'].sum(axis=0)
    for state in range(mdp_data['transition_counts'].shape[1]):
        for action in range(2):
            if total_count[state, action] > 0:
                for new_state in range(mdp_data['transition_counts'].shape[0]):
                    mdp_data['transition_probs'][new_state, state, action] = \
                        mdp_data['transition_counts'][new_state, state, action] / \
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    num_iter = 0
    max_change_list = np.zeros(mdp_data['transition_probs'].shape[1])
    while max_change > tolerance:
        num_iter += 1
        for state in range(mdp_data['transition_probs'].shape[1]):
            value = np.zeros(2)
            for idx_action in range(2):
                for new_state in range(mdp_data['transition_probs'].shape[0]):
                    value[idx_action] += mdp_data['transition_probs'][new_state, state, idx_action] * \
                                         mdp_data['value'][new_state]

            value_orig = mdp_data['value'][state]
            mdp_data['value'][state] = mdp_data['reward'][state] + gamma * max(value)
            max_change_list[state] = np.abs(value_orig - mdp_data['value'][state])
        max_change = max(max_change_list)
    return(num_iter < 2)
    # *** END CODE HERE ***
